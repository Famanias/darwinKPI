  ...
  async (req, res) => {
    try {
      ...
      if (!jsonData || !jsonData.length) {
        return res
          .status(400)
          .json({ message: "File is empty or contains no valid data." });
      }

      // Validate data structure
      if (
        !jsonData[0].hasOwnProperty("date") ||
        !jsonData[0].hasOwnProperty("value")
      ) {
        return res.status(400).json({
          message:
            'Invalid file format. File must contain "date" and "value" columns.',
        });
      }
      ...
      for (const [index, row] of jsonData.entries()) {
        ...
        if (!row.date || !isValidDate(row.date)) {
          return res.status(400).json({
            message: `Invalid date format at row ${
              index + 2
            }. Expected format: YYYY-MM-DD or ISO 8601 format.`,
          });
        }
        ...
        if (isNaN(value)) {
          return res.status(400).json({
            message: `Invalid numeric value at row ${
              index + 2
      ...
      // Build insert values with org_id
      ...
      // Validate KPI exists and belongs to user's organization
      const kpi = await db.getAsync(
        "SELECT id FROM kpis WHERE id = ? AND org_id = ?",
        [kpi_id, req.user.org_id]
      );
      if (!kpi) {
        return res
          .status(400)
          .json({
            message:
              "Selected KPI does not exist or is not in your organization.",
          });
      }

      // Insert data using prepared statements (SQLite doesn't support bulk insert like MySQL)
      let insertedCount = 0;
